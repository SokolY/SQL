...
Create Table Pupils(
	...
	--ця таблиця не потрібна, якщо не має мети зберігати інформацію, наприклад, щодо в яких класах ВЗАГАЛІ навчався учень.
	-- пояснюю: в конкретний момент часу один учень навчається лише в одному класі. а вкласі - багато учнів. один до багатьох зв'зок - технічна таблиця не потрібна.
		-- можна було б просто розмістити референс кі в на класс в таблиці з учнями.
		-- але. якщо ми маємо на меті зберігати інформацію про класи в яких навчався учень впродовж свого освітнього шляху - то виникає зв'язок багато-до багтьох і твоя таблиця вже може існувати, аде з додатковими полями типу "коли поступив" "коли випустився" "примітки" тощо.
...
-- тут би я хотів щоб ще додавалась інформація в таблицю про зарплатню, але ок.
