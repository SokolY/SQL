...
--товар збігаються з вже існуючими даними (постачальник, ім'я, категорія, ціна), замість додавання відбувається оновлення інформації про кількість товару.
	...
	Where Products.ProductName = 'ABavlova'

	Select * From Products
	Where Products.UnitsInStock = 33
	...
	values ('ABavlova', 7, 3, '32 - 500 g boxes', 17.45, 33, 5, 15, 0)

	Select *
	From ProductsTemp
	Where ProductsTemp.ProductName = 'Pavlova'

	drop trigger ProductCheck
	Go
	...
	Begin
		Declare
		@vProductName nvarchar(50) = (Select inserted.ProductName from inserted)
		Declare
		@vProdExist int = (Select count(*)
			From Products PT
			Where PT.ProductName like @vProductName)
		Declare
	...
	Select * -- тут робив копію таблиці і з нею працював, щоб не поламати основну таблицю
	into EmployeesTemp
	From Employees
	...
	--3. Тригер забороняє додавати нового постачальника, якщо кількість існуючих постачальників більше 28.
	...
	Begin
	if ((Select Count(*) 
	From SuppliersTemp)>28)
		begin
		Print 'To many Suppliers. It is forbiden add new'
		end
	else insert into Suppliers (CompanyName, ContactName)
			select CompanyName, ContactName From inserted
	End
	Go
	...
	Delete
	From Suppliers 
	Where Suppliers.CompanyName like 'Yurii LTD'
